...
describe("WebSocket Runner", () => {
  ...
  it("should handle error messages correctly", async () => {
    ...
    expect(console.error).toHaveBeenCalledWith(expect.stringContaining("WebSocket error"))
  ...
  it("should perform cleanup on exit", async () => {
    ...
    const exitHandler = mockProcessOn.mock.calls.find(call => call[0] === "SIGINT")[1]
    ...
    expect(console.error).toHaveBeenCalledWith(expect.stringContaining("Shutting down WS Runner"))
  })
