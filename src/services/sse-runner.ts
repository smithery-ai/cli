...
export class SSERunner {
	...
	private async reconnect(): Promise<void> {
		if (this.reconnectAttempts >= this.MAX_RECONNECT_ATTEMPTS) {
			console.error(
				`Max reconnection attempts (${this.MAX_RECONNECT_ATTEMPTS}) reached, exiting...`,
			)
			console.error(
				`Last parsed message: ${JSON.stringify(this.lastParsedMessage, null, 2)}`,
			)
			process.exit(1)
			return
		}
		...
		try {
			...
			console.error(
				`Last parsed message before failure: ${JSON.stringify(this.lastParsedMessage, null, 2)}`,
			)
		}
	}
	...
	async processMessage(input: Buffer): Promise<void> {
		...
		try {
			...
			console.error(
				`Note: Message contains multiple JSON objects or is malformed`,
			)
		}
