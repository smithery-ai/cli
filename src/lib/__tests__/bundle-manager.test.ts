...
describe('Bundle Manager', () => {
	...
	describe('resolveTemplateString', () => {
		it('should resolve __dirname template', () => {
			const result = resolveTemplateString('${__dirname}/server.js', {}, '/bundle/path')
		...
		it('should resolve nested user_config template', () => {
			...
			const result = resolveTemplateString('${user_config.database.host}', userConfig)
		...
		it('should handle multiple templates in one string', () => {
			...
			const result = resolveTemplateString('${user_config.host}:${user_config.port}', userConfig)
	...
	describe('getBundleCommand', () => {
		...
		it('should throw error for missing command', async () => {
			...
			expect(() => getBundleCommand(tempDir)).toThrow('Bundle manifest missing server.mcp_config.command')
		})
	...
	describe('Template Resolution Integration', () => {
		it('should handle complete manifest with args and env resolution', async () => {
			...
			const resolvedArgs = args.map(arg => resolveTemplateString(arg, userConfig, tempDir))
