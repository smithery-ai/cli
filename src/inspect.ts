...
async function createClient() {
  const client = new Client({ name: "smithery-cli", version: "1.0.0" }, { capabilities: {} })
  ...
  try {
    ...
    while (true) {
      ...
      if (primitive.type === "resource") {
        ...
        console.log(chalk.dim(`Description: ${primitive.value.description || 'No description'}`))
        console.log(chalk.cyan('\nInput Schema:'))
    ...
    spinner.fail(`Failed to connect to server: ${error instanceof Error ? error.message : String(error)}`)
...
async function readJSONSchemaInputs(schema: any) {
  ...
  const questions: Array<{key: string, required?: boolean, type: string, [key: string]: any}> = []
...
async function readPromptArgumentInputs(args: any[]) {
  ...
  return inquirer.prompt(
    args.map((arg) => ({
      type: "text",
      name: arg.name,
      message: chalk.dim((arg.required ? "* " : "") + `${arg.name}: ${arg.description}`),
    }))
  )
}
