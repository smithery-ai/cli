...
const mockHttpServer = {
  ...
  remote: true,
  connections: [
    { type: 'http', deploymentUrl: 'https://server.smithery.ai/test-server/mcp' }
  ]
...
const mockStdioServer = {
  ...
  remote: false,
  connections: [
    { type: 'stdio', command: 'npx', args: ['test-server'] }
  ]
} as unknown as ServerDetailResponse
...
const EXPECTED_OUTPUTS = {
  ...
  'command-stdio': [
    "--add-server", "test-server", "npx", "test-server"
  ],
  'command-http': [
    "mcp", "add", "--transport", "http", "test-server", "https://server.smithery.ai/test-server/mcp"
  ],
  'yaml-stdio': {
    command: "npx",
    args: ["-y", "@smithery/cli@latest", "run", "test-server", "--key", "test-api-key"]
...
describe('Server Installation', () => {
  ...
  test('JSON + STDIO', () => {
    ...
    const result = mockFormatServerConfig('test-server', {}, 'test-api-key', undefined, config, mockStdioServer)
  ...
  test('JSON + HTTP', () => {
    ...
    const result = mockFormatServerConfig('test-server', {}, 'test-api-key', undefined, config, mockHttpServer)
  ...
  test('Command + STDIO', () => {
    ...
    const stdioOutput = config.commandConfig?.stdio?.('test-server', 'npx', ['test-server'])
  ...
  test('Command + HTTP', () => {
    ...
    const httpOutput = config.commandConfig?.http?.('test-server', 'https://server.smithery.ai/test-server/mcp')
